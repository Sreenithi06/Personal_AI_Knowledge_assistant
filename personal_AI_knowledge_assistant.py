...
def google_search_links(query, num_results=3, pause=1.0):
    """Lightweight Google HTML search scraping. Returns list of absolute URLs."""
    ...
    links = []
    # Google search returns /url?q=<actual_url>&sa=... anchors
...
def fetch_page_text(url, timeout=8):
    ...
    try:
        ...
        # take the longer paragraphs only
...
def build_context_from_links(links):
    ...
    for u in links:
        ...
        if txt:
            # keep a small window from each source
...
def ask_openai_system(query, context, mode='brief'):
    ...
    try:
        resp = client.chat.completions.create(
            model="gpt-4.1-mini",   # use gpt-4.1, gpt-4.1-mini, gpt-3.5-fallback etc
            messages=[
...
def ask_route():
    ...
    if not context:
        # if we couldn't fetch any page text, still send question without context
